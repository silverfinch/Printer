	...
	def isInside(self,other):
		...
		for thisLine in self.lines:
			for otherLine in other.lines:
				found = 0
				if thisLine.isInteriorTo(otherLine):
					found = 1
					break
			if (found == 0):
				return 0# if any of thisLines aren't interior to any otherLines, then interior state is false, end function
		return 1
...
#		print(len(badLines))
	...
	def trimShell(self,baseContour,LINEWIDTH):
		...
		while i>=0:# delete lines too close to base contour
			...
			i-=1
		#now join the lines left
#		i = len(self.lines)-1
#		while i>=0:
#			index = None
#			j = i-1
#			while (j>=1) or (j>=0 and i<len(self.lines)-1):
#				if self.lines[j].intersectsWith(self.lines[i]) is not None:#if lines intersect
#					index = j
#				j -= 1
#			if index is not None:
#				[x,y] = self.lines[index].intersectsWith(self.lines[i])
#				self.lines[index].endPoint.X = x#connect the lines
#				self.lines[index].endPoint.Y = y
#				self.lines[i].startPoint.X = x
#				self.lines[i].startPoint.Y = y
#				print('poop')
#				print(i)
#				print(index+1)
#				del self.lines[index+1:i]#delete all lines between
#				i = index#move index past lines just completed
#			i -= 1
